...
export class EdituserComponent implements OnInit {
  ...
  statusMsg:string;
   ...
   states = [];
   cities = [];
  constructor(private fb:FormBuilder,private userlistserice:UsertableService,private route:Router, private route2: ActivatedRoute,private router:Router) {
    this.createeditForm(); 
    this.edituserId=this.route2.snapshot.paramMap.get("id"); 
  ...
  ngOnInit() {
    ...
    this.userlistserice.getuserByID(this.edituserId).subscribe(
     data => {
      this.empData=data['user']; 
      this.selectedCountry=   this.empData.country;
      this.selectedState=this.empData.state;
      this.selectedCity=this.empData.city;
      this.states = this.getStates().filter((item) => {
        return item.country_id === Number( this.selectedCountry)
      });
      this.cities = this.getCity().filter((item) => {
        return item.state_id === Number(this.selectedState)
      });
    });
  ...
  {  
    ...
    someFormData.append('id',this.edituserId);
    console.log(someFormData);
    this.userlistserice.userUpdate(someFormData).subscribe(data => {
      console.log(data);
